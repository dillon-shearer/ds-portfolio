...
const SEMANTIC_MAPPINGS = [
  ...
  },
  {
    ...
    sql: 'Use a sets CTE. Compute set_order with COALESCE(set_number, ROW_NUMBER() OVER (PARTITION BY session_date, exercise ORDER BY performed_at)). Bucket with NTILE(3) over set_order to compare early/mid/late averages, and surface best vs worst sets by weight or estimated 1RM.',
  },
  {
...
export const SEMANTIC_HINTS = SEMANTIC_MAPPINGS.map(
  ({ phrase, sql }) => `- ${phrase} => ${sql}`,
).join('\n')
